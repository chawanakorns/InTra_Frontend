...
export default function SignUp() {
  ...
  const validateForm = () => {
    ...
    if (!validateEmail(email.trim())) { Alert.alert("Error", "Please enter a valid email address"); return false; }
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={{ marginTop: 30 }}><Text style={styles.labelText}>Full Name</Text><TextInput style={styles.input} value={fullName} onChangeText={setFullName} placeholder="Enter Your Full Name" autoCapitalize="words"/></View>
      <View style={styles.row}>
        ...
        <View style={{ width: "48%" }}>
          ...
          <Dropdown style={styles.dropdown} placeholderStyle={styles.placeholderStyle} selectedTextStyle={styles.selectedTextStyle} data={genders} maxHeight={300} labelField="label" valueField="value" value={gender} onChange={(item) => setGender(item.value)} renderRightIcon={() => (<Ionicons name="chevron-down" size={20} color={Colors.GRAY} />)}/>
        </View>
      </View>
      <View style={{ marginTop: 10 }}><Text style={styles.labelText}>Email Address</Text><TextInput style={[styles.input, !isEmailValid && styles.inputError]} value={email} onChangeText={handleEmailChange} keyboardType="email-address" autoCapitalize="none" placeholder="Enter Your E-mail (e.g., test@email.com)" />{!isEmailValid && (<Text style={styles.errorText}>Please enter a valid email address</Text>)}</View>
      <View style={{ marginTop: 10 }}><Text style={styles.labelText}>Password</Text><TextInput secureTextEntry={true} style={styles.input} value={password} onChangeText={setPassword} placeholder="Enter Your Password" /></View>
...
const styles = StyleSheet.create({
  ...
  subtitle: { fontFamily: "outfit", fontSize: 16, color: Colors.WHITE, marginTop: 10 },
  labelText: { fontFamily: "outfit", fontSize: 16, color: Colors.WHITE, marginBottom: 10 },
  input: { padding: 15, borderWidth: 1, borderRadius: 15, borderColor: Colors.GRAY, backgroundColor: Colors.WHITE, fontFamily: 'outfit', fontSize: 16 },
  dateInput: { padding: 15, borderWidth: 1, borderRadius: 15, borderColor: Colors.GRAY, backgroundColor: Colors.WHITE, flexDirection: "row", justifyContent: "space-between", alignItems: "center", height: 58 },
  dateText: { fontFamily: 'outfit', fontSize: 16, color: Colors.BLACK, },
  row: { flexDirection: "row", justifyContent: "space-between", marginTop: 10 },
  dropdown: { paddingHorizontal: 15, borderWidth: 1, borderRadius: 15, borderColor: Colors.GRAY, backgroundColor: Colors.WHITE, height: 58 },
  placeholderStyle: { fontFamily: "outfit", fontSize: 16, color: Colors.BLACK },
  selectedTextStyle: { fontFamily: "outfit", fontSize: 16, color: Colors.BLACK },
  inputError: { borderColor: "#FF0000", borderWidth: 2 },
  errorText: { color: "#FF0000", fontSize: 12, fontFamily: "outfit", marginTop: 5 },
  button: { padding: 15, borderRadius: 15, marginTop: 40, backgroundColor: Colors.PRIMARY, flexDirection: "row", justifyContent: "center", alignItems: "center" },
